...
/**
 * It represents OSLC JIRA Change Request. It serves as data class which is
 * sending between OSLC system It extended attributes beyond OSLC base
 * ChangeRequest
 */
...
public final class JiraChangeRequest extends ChangeRequest {
	...
	// -------------- REQUIREMENT -----------------------
	@OslcDescription("The Jira issue requirement for this change request")
	@OslcOccurs(Occurs.ZeroOrOne)
	@OslcPropertyDefinition(Constants.JIRA_NAMESPACE + "requirement")
	...
	public static JiraChangeRequest fromJiraIssue(MutableIssue issue) throws URISyntaxException {
		...
		if (status != null) {
			...
			jStatus.setAbout(new URI(JiraManager.getRestUrl() + JiraConstants.ISSUE_STATUS_PATH + issue.getKey() + "/"
					+ status.getId()));
			jStatus.setIssueStatusName(status.getName());
		...
		if (issueType != null) {
			...
			jiraIssueType.setIssueTypeName(issueType.getName());
			// jiraIssueType.setAbout(new URI(JiraManager.getRestUrl() +
			// JiraConstants.ISSUE_TYPE_PATH + id));
		...
		// Note: field for related change requests is in base class
		// ChangeRequest
		...
		if (priority != null) {
			...
			jiraIssuePriority
					.setAbout(new URI(JiraManager.getRestUrl() + JiraConstants.ISSUE_PRIORITY_PATH + priority.getId()));
		...
		history.setAbout(new URI(
				JiraManager.getRestUrl() + issue.getProjectId() + "/changeRequests/" + issue.getKey() + "/history"));
		jcr.setIssueHistory(history);

		// voting
		List<String> voters = new ArrayList<String>();
		VoteManager vMngr = ComponentAccessor.getVoteManager();
		List<ApplicationUser> v = vMngr.getVotersFor(issue,
		...
		List<ApplicationUser> wList = watcherMngr.getWatchers(issue,
				ComponentAccessor.getJiraAuthenticationContext().getLocale());
		...
		for (IssueLink link : inwards) {
			...
			JiraIssueInternalLink jiil = new JiraIssueInternalLink(OSLCUtils.getRestUriForIssue(srcIssue),
					type.getInward(), "inward");
		...
		for (IssueLink link : outwards) {
			...
			JiraIssueInternalLink jiil = new JiraIssueInternalLink(OSLCUtils.getRestUriForIssue(dstIssue),
					type.getOutward(), "outward");
		...
		for (CustomField cf : cfList) {
			// skip "External Links" custom field, because content is exported
			// as related change request(s)
			if (cf.getName().equals("Requirement")) {
				continue;
			}
		...
		// ---------REQUIREMENT--------
		String issueRequirement = "";
		for (CustomField cf : cfList) {
			if (cf.getName().equals("Requirement")) {
				JiraIssueCustomField jicf = new JiraIssueCustomField(cf, issue);
				issueRequirement = jicf.getValue();
			}
		}
		jcr.setRequirement(issueRequirement);
	 ...
	 * @param leanSyncConfiguration
	 *            LeanSync configuration. If the syncType is null the
	...
	public static JiraIssueInputParameters toIssueParameters(JiraChangeRequest jcr, MutableIssue issue,
			Map<String, Object> selectedProperties, String syncType, SyncConfiguration leanSyncConfiguration, Project p)
			throws Exception {
		...
		// OSLC Links
		if (OSLCUtils.allowUpdate(selectedProperties, Constants.TYPE_RELATED_CHANGE_REQUEST, syncType,
				true & isCreate)) {
			Link[] oldLinks = null;
			CustomFieldManager customFieldManager = ComponentAccessor.getCustomFieldManager();
			CustomField customField = customFieldManager
					.getCustomFieldObjectByName(JiraConstants.OSLC_CUSTOM_FIELD_NAME);
			...
			if (customField != null && relatedChangeRequests != null) {
				...
				// We have to use the title from the current value of
		...
		// Note: in case of creating new issue, setting components, affect and
		// fix versions is
		// postponed, after issue is successfully created (see
		// JiraManager.createIssue).
		...
		// COMMENT
		List<JiraIssueComment> comments = jcr.getIssueComments();
		if (comments != null) {
			for (JiraIssueComment comm : comments) {
				jiraIssueInputParams.getIssueInputParameters().setComment(comm.getCommentBody());
			}
		...
		// Instead they are set to successfully created issue (see
		// JiraManager.updateIssue).

		return jiraIssueInputParams;
	}

	/**
	 * It add the values jiraIssueInputParams according to LeanSync
	 * configuration
	 * 
	 * @param jcr
	 *            JIRA Change Request
	 * @param issue
	 *            JIRA issue
	 * @param jiraIssueInputParams
	 *            JIRA IssueInputParameters
	 * @param leanSyncConfig
	 *            LeanSync configuration
	 * @param isCreate
	 *            true - JIRA issue will be create, false - the JIRA issue will
	 *            be updated
	 * @throws Exception
	 */
	private static void handleLeanSync(JiraChangeRequest jcr, MutableIssue issue,
			JiraIssueInputParameters jiraIssueInputParams, SyncConfiguration leanSyncConfig, boolean isCreate)
			throws Exception {
	 ...
	 * 
	 * @param uri
	 *            the uri containing if in the last segment of uri
